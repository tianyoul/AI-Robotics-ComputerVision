    ...
    def userKeyResponse(self):
        ...
        if chrKey in {'q', 'Q'}:
            ...
            if self.inAir:
                ...
                self.inAir = True
        elif chrKey in {'i', 'I'}:  # Moving up
            if self.inAir:
                self.drone.move_up()
                time.sleep(0.3)
                self.drone.hover()
            explanText = "Moving up"
        elif chrKey in {'k', 'K'}:   # Moving down
            if self.inAir:
                self.drone.move_down()
                time.sleep(0.3)
                self.drone.hover()
            explanText = "Moving down"
        elif chrKey in {'j', 'J'}:    # Turning left
            if self.inAir:
                self.drone.turn_left()
                time.sleep(0.3)
                self.drone.hover()
            explanText = "Turning left"
        elif chrKey in {'l', 'L'}:    # Turning right
            if self.inAir:
                self.drone.turn_right()
                time.sleep(0.3)
                self.drone.hover()
            explanText = 'Turning right'
        elif chrKey in {'w', 'W'}:    # Moving forward
            if self.inAir:
                self.drone.move_forward()
                time.sleep(0.3)
                self.drone.hover()
            explanText = 'Moving forward'
        elif chrKey in {'s', 'S'}:    # Moving backward
            if self.inAir:
                self.drone.move_backward()
                time.sleep(0.3)
                self.drone.hover()
            explanText = 'Moving backward'
        elif chrKey in {'a', 'A'}:    # Moving left
            if self.inAir:
                self.drone.move_left()
                time.sleep(0.3)
                self.drone.hover()
            explanText = 'Moving left'
        elif chrKey in {'d', 'D'}:    # Moving right
            if self.inAir:
                self.drone.move_right()
                time.sleep(0.3)
                self.drone.hover()
            explanText = 'Moving right'
        elif chrKey in {'1', '2', '3', '4', '5', '6', '7', '8', '9', '0'}:    # Changing speed
            newSpeed = self.keyToSpeed(chrKey)
            self.drone.speed = newSpeed
            explanText = 'Changing speed to ' + str(newSpeed)
        else:
            explanText = "Unknown key"
